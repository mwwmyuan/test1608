...
extern uint8_t app_a2dp_source_find_process;
...
//void bt_key_handle_pair(enum APP_KEY_EVENT_T)
...
//#endif
    switch(event)
    {
        ...
        default:
            TRACE("unregister up key event=%x\n",event);
            break;
    }
}
...
void bt_key_handle_source_func_key(enum APP_KEY_EVENT_T event)
{
 ...
 /*#if defined(APP_LINEIN_A2DP_SOURCE)||defined(APP_I2S_A2DP_SOURCE)
		...
		case APP_KEY_EVENT_DOUBLECLICK:
			if(*(app_bt_device.a2dp_state)==0){
				if(app_a2dp_source_find_process == 0){
		    		//app_status_indication_set(APP_STATUS_INDICATION_SRC_SEARCH);
					TRACE("!!!!!!!!!!!!!!!!!!!enter search!!!!!!!!!!!!!!!!!!!!\n");
					TRACE("!!!!!!!!!!!!!!!!!!!%d!!!!!!!!!!!!!!!!!!!!\n",app_a2dp_source_find_process);
					app_a2dp_source_find_sink();
					app_status_indication_set(APP_STATUS_INDICATION_SRC_SEARCH);
					}
				}
