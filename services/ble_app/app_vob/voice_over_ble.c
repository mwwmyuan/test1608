 ...
 ****************************************************************************************
 *
 * @file voice_over_ble.c
 *
 * @date 27 Mar 2017
 *
 * @brief The application thread specially for the voice over ble.
 *
 * Copyright (C) 2017
 *
 *
...
uint32_t a2dp_audio_prepare_pcm_data(uint8_t *buf, uint32_t len)
{
	...
	{
		uint32_t acquiredPCMDataLength;
	    if (allow_resample){
	        int res;
	        //    int32_t stime,etime;
	        ...
	        do{
	            acquiredPCMDataLength = decode_sbc_frame((uint8_t *)raw_pcm_data, SAMPLE_NUM*2*2);
	            if (acquiredPCMDataLength < SAMPLE_NUM*2*2){            
	        ...
	        acquiredPCMDataLength = decode_sbc_frame(buf, len);
...
#endif	// #ifdef __AUDIO_RESAMPLE__
