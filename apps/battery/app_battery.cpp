...
#define APP_LINEOUT_PERIODIC_MS (2200)
...
static void app_lineout_timehandler(void const *param)
{
  bool lineout_cfg = !hal_gpio_pin_get_val((enum HAL_GPIO_PIN_T)app_line_out_detect_cfg.pin);
  bool linein_cfg = !hal_gpio_pin_get_val((enum HAL_GPIO_PIN_T)app_line_in_detect_cfg.pin);
	if (app_line_out_detect_cfg.pin != HAL_IOMUX_PIN_NUM){
		if ((lineout_cfg)&&!(linein_cfg)){
...
int app_battery_open(void)
{
	...
	if (app_lineout_timer == NULL)
        app_lineout_timer = osTimerCreate (osTimer(APP_LINEOUT), osTimerPeriodic, NULL);
