...
{
   	hal_iomux_init((struct HAL_IOMUX_PIN_FUNCTION_MAP *)&app_line_out_detect_cfg, 1);
    hal_gpio_pin_set_dir((enum HAL_GPIO_PIN_T)app_line_out_detect_cfg.pin, HAL_GPIO_DIR_IN, 1);
	hal_iomux_init((struct HAL_IOMUX_PIN_FUNCTION_MAP *)&app_line_in_detect_cfg, 1);
    hal_gpio_pin_set_dir((enum HAL_GPIO_PIN_T)app_line_in_detect_cfg.pin, HAL_GPIO_DIR_IN, 1);
}
...
int app_deinit(int deinit_case)
{
    ...
    if (!deinit_case){
	   ...
	   app_audio_sendrequest((uint8_t)APP_A2DP_SOURCE_LINEIN_AUDIO, (uint8_t)APP_BT_SETTING_CLOSE,0);
		...
		app_status_indication_filter_set(APP_STATUS_INDICATION_SRC_CONNECTABLE);
