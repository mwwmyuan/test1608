...
ifneq ($(wildcard scripts/timediff.bat),)
	@scripts/timediff.bat "%time%" "$(START_TIME)"
endif
else
...
targets := $(LDS_TARGET) $(IMAGE_FILE) $(IMAGE_BIN) $(IMAGE_LST) $(IMAGE_VER)
...
all: $(IMAGE_BIN) ;
      ...
      cmd_gen-IMAGE_BIN = $(OBJCOPY) -O binary $< $@
...
ifeq ($(OTA_BUILD),1)
BUILD_INFO_FLAGS += -DOTA_BUILD -DOTA_SEC_SIZE=$(FLASH_REGION_SIZE)
endif
