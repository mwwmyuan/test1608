...
void app_bt_det(APP_DET_STATUS *status, void *param)
{
		TRACE("%s %d,%d",__func__, status->code, status->event);
		switch(status->event)
		{
			case APP_DET_EVENT_UP:
				TRACE("rising!");
				break;
			case APP_DET_EVENT_DOWN:
				TRACE("falling");
				break;				  
		}
		bt_det_send(status);
...
const APP_KEY_HANDLE  app_key_handle_cfg[] = {
    ...
    //{{APP_KEY_CODE_FN3,APP_KEY_EVENT_UP},"line-in key",app_bt_key, NULL},
   // {{APP_KEY_CODE_FN4,APP_KEY_EVENT_UP},"line-out key",app_bt_key, NULL},
#if HF_CUSTOM_FEATURE_SUPPORT & HF_CUSTOM_FEATURE_SIRI_REPORT
    {{APP_KEY_CODE_NONE ,APP_KEY_EVENT_NONE},"none function key",app_bt_key, NULL},
#endif
};
...
const APP_DET_HANDLE  app_det_handle_cfg[] = {
    {{APP_DET_CODE_FN1,APP_DET_EVENT_UP},"line in detect",app_bt_det, NULL},
    {{APP_DET_CODE_FN2,APP_DET_EVENT_UP},"line out detect",app_bt_det, NULL},
...
void app_det_init(void)
{
    uint8_t i = 0;
    TRACE("%s",__func__);
    
    app_det_handle_clear();
    for (i=0; i<(sizeof(app_det_handle_cfg)/sizeof(APP_DET_HANDLE)); i++){
        app_det_handle_registration(&app_det_handle_cfg[i]);
    }
}
