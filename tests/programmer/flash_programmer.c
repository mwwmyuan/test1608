...
static enum XFER_TIMEOUT recv_extra_timeout;
...
static void init_data_buf_state(void)
{
    unsigned int i;
...
int handle_data_buf(unsigned int index)
{
    ...
    TRACE_TIME("### FLASH_TASK: %s index=%d ###", __FUNCTION__, index);
    ...
    TRACE("### FLASH_TASK: sec_seq=%d, addr=0x%08x, len=%d ###", sec_seq, addr, len);

    if (programmer_state == PROGRAMMER_ERASE_BURN_START) {
        ret = erase_sector(addr, len);
        if (ret) {
            TRACE("### FLASH_TASK: ERASE_DATA failed: addr=0x%08x, len=%d ###", addr, len);
    ...
    if (ret) {
        TRACE("### FLASH_TASK: BURN_DATA failed: addr=0x%08x, len=%d ###", addr, len);
        finish_data_buf(index, ERR_BURN_FLSH);
        return 0;
    }
    TRACE_TIME("### FLASH_TASK: Burn done ###");
#if defined(__EXT_CMD_SUPPORT__)
    memcpy32((uint32_t *)&data_buf[index][BURN_DATA_MSG_OVERHEAD], (uint32_t *)addr, len/sizeof(uint32_t));
    crc = CRC32_FUNC(0, &data_buf[index][BURN_DATA_MSG_OVERHEAD], len);
    if (crc != mcrc) {
        TRACE("### FLASH_TASK: VERIFY_DATA failed: bad CRC");
        finish_data_buf(index, ERR_VERIFY_FLSH);
        return 0;
    }
#else
    crc = CRC32_FUNC(0, (unsigned char *)addr, len);
    if (crc != mcrc) {
        TRACE("### FLASH_TASK: VERIFY_DATA failed: bad CRC");
        finish_data_buf(index, ERR_VERIFY_FLSH);
        fdata = (unsigned char *)addr;
        mdata = &data_buf[index][BURN_DATA_MSG_OVERHEAD];
        for (i = 0; i < len; i++) {
            if (*fdata++ != *mdata++) {
                --fdata;
                --mdata;
                TRACE("### FLASH_TASK: 0x%02x @ %p != 0x%02x @ %p", *fdata, fdata, *mdata, mdata);
                break;
            }
        }
        if (i == len) {
            TRACE("### FLASH_TASK: Byte by byte check OK");
        }
        return 0;
    }
#endif    
...
static void init_parse_state(unsigned char **buf, size_t *len)
{
...
static void init_programmer_state(unsigned char **buf, size_t *len)
{
    ...
    switch (recv_msg.hdr.type) {
        ...
        case TYPE_BURN_START:
            if (recv_msg.hdr.len != 12) {
                //TRACE("BURN_START %d msg length error: %d", recv_msg.hdr.type, recv_msg.hdr.len);
                errcode = ERR_LEN;
            }
            break;
        ...
        case TYPE_BURN_CMD:
            if (recv_msg.hdr.len != 1 && (recv_msg.hdr.len <= 5 || recv_msg.hdr.len > 21)) {
                //TRACE("BURN_CMD msg length error: %d", recv_msg.hdr.len);
                errcode = ERR_LEN;
            }
            break;
        case TYPE_GET_SECTOR_INFO:
            if (recv_msg.hdr.len != 4) {
                //TRACE("GET_SECTOR_INFO msg length error: %d", recv_msg.hdr.len);
                errcode = ERR_LEN;
            }
            break;
#if defined(__EXT_CMD_SUPPORT__)
			case TYPE_EXTEND_CMD: {
				errcode = extend_cmd_check_msg_hdr(&recv_msg);
				break;
			}
#endif			
        default:
...
static enum ERR_CODE handle_burn_cmd(enum BURN_CMD_TYPE cmd, unsigned char *param, unsigned int len)
{
    ...
    switch (cmd) {
        case BURN_CMD_ERASE_SECTOR: {
            ...
            if (len != 8) {
                TRACE("Invalid ERASE_SECTOR cmd param len: %d");
                return ERR_LEN;
            }

            addr = param[0] | (param[1] << 8) | (param[2] << 16) | (param[3] << 24);
            size = param[4] | (param[5] << 8) | (param[6] << 16) | (param[7] << 24);
            TRACE("addr=0x%08x, size=%d", addr, size);

            ret = erase_sector(addr, size);
            if (ret) {
                TRACE("ERASE_SECTOR failed: addr=0x%08x, size=0x%08x", addr, size);
        ...
        case BURN_CMD_BURN_DATA: {
            ...
            if (len <= 4 || len > 20) {
                TRACE("Invalid BURN_DATA cmd param len: %d", len);
                return ERR_LEN;
            }

            addr = param[0] | (param[1] << 8) | (param[2] << 16) | (param[3] << 24);
            TRACE("addr=0x%08x, len=%d", addr, len - 4);

            ret = burn_data(addr, &param[4], len - 4);
            if (ret) {
                TRACE("BURN_DATA failed: addr=0x%08x, len=%d", addr, len - 4);
        ...
        case BURN_CMD_ERASE_CHIP: {
            ...
            if (len != 0) {
                TRACE("Invalid ERASE_CHIP cmd param len: %d", len);
                return ERR_LEN;
            }
            ret = erase_chip();
            if (ret) {
        ...
        default:
            TRACE("Unsupported burn cmd: %d", cmd);
            return ERR_BURN_CMD;
    }
...
static enum ERR_CODE handle_sector_info_cmd(unsigned int addr)
{
    ...
    TRACE("sector_addr=0x%08x, sector_len=%d", sector_addr, sector_len);
    ...
    *extra = 0;
    ...
    switch (recv_msg.hdr.type) {
        ...
        case TYPE_BURN_START: {
            ...
            TRACE("burn_addr=0x%08x, burn_len=%d, sector_size=%d", burn_addr, burn_len, sector_size);

            if ((size_mask & sector_size) == 0 || count_set_bits(sector_size) != 1) {
                TRACE("Unsupported sector size: 0x%08x, mask: 0x%08x", sector_size, size_mask);
            ...
            *len = BURN_DATA_MSG_OVERHEAD + ((sector_cnt == 1) ? last_sector_len : sector_size);
            ...
            recv_extra_timeout = XFER_TIMEOUT_LONG;
            break;
        }
        case TYPE_BURN_CMD: {
...
#if defined(__EXT_CMD_SUPPORT__)
			case TYPE_EXTEND_CMD: {
				TRACE_TIME("------ EXTEND CMD ------");
				errcode = extend_cmd_handle_cmd((enum SYS_CMD_TYPE)recv_msg.data[0], &recv_msg.data[1], recv_msg.hdr.len - 1);
				if (errcode != ERR_NONE) {
					return errcode;
				}
				break;
...
#endif			
        default:
            TRACE("Invalid message type: %d", recv_msg.hdr.type);
...
static enum ERR_CODE check_extra_burn_data_msg(unsigned char *data)
{
    ...
    if (msg->hdr.prefix != PREFIX_CHAR) {
        ...
        TRACE("Invalid %s msg type: %d", str_burn_data, msg->hdr.type);
        errcode = ERR_TYPE_INVALID;
    } else if (msg->hdr.len != 11) {
        TRACE("%s msg length error: %d", str_burn_data, msg->hdr.len);
        errcode = ERR_LEN;
    ...
    *extra = 0;
    ...
    switch (recv_msg.hdr.type) {
        ...
        case TYPE_BURN_START: {
            ...
            TRACE("%s: sec_seq=%d, dlen=%d, cur_data_buf_idx=%d", str_burn_data, cur_sector_seq, dlen, cur_data_buf_idx);

            if (cur_sector_seq >= sector_cnt) {
                TRACE("%s: Bad sector seq: sec_seq=%d, sector_cnt=%d", str_burn_data, cur_sector_seq, sector_cnt);
            ...
            if (((cur_sector_seq + 1) == sector_cnt && dlen != last_sector_len) ||
                ...
                TRACE("%s: Bad sector len: sec_seq=%d, dlen=%d", str_burn_data, cur_sector_seq, dlen);
            ...
            if (cur_sector_seq + 1 == sector_cnt) {
                ...
                TRACE_TIME("%s: Recv next buffer. cur_data_buf_idx=%d", str_burn_data, cur_data_buf_idx);
...
_sync:
    ...
    while (1) {
        ...
        if (parse_state == PARSE_EXTRA) {
            ...
            ret = recv_data_dma(buf, extra_buf_len, len, &rlen);
        } else {
            if (programmer_state == PROGRAMMER_NONE && parse_state == PARSE_HEADER) {
                set_recv_timeout(XFER_TIMEOUT_IDLE);
            } else {
                set_recv_timeout(XFER_TIMEOUT_SHORT);
            }
            ret = recv_data(buf, len, &rlen);
        }
        if (ret) {
        ...
        if (len != rlen) {
            TRACE("Receiving part of the data: expect=%d, real=%d", len, rlen);
            goto _err;
        }
