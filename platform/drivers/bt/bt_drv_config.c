...
extern void btdrv_send_cmd(uint16_t opcode,uint8_t cmdlen,uint8_t *param);
...
{
    0xBF,0xFE,0xCD,0xFa,0xDB,0xFd,0x7b,0x87
};
...
void btdrv_bt_spi_setneg_rawbuf(uint8_t wr_type, uint8_t *spirawbuf, uint8_t pos_len,uint8_t neg_len)
{
	if (1 == wr_type)
...
void btdrv_bt_spi_setpos_rawbuf(uint8_t wr_type, uint8_t *spirawbuf, uint8_t pos_len)
{
	if (1 == wr_type)
