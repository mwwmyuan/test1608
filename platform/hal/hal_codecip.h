...
static inline void codecip_w_dac_down_sel(uint32_t reg_base, uint32_t v)
{
    uint32_t val = 0;
    val = codecip_read32(reg_base, CODECIP_CTRL1_REG_OFFSET);
...
static inline void codecipbus_w_dac_en(uint32_t reg_base, uint32_t v)
{
    uint32_t val = 0;
    val = codecip_read32(reg_base, CODECIP_CR_REG_OFFSET);
    if (v)
...
static inline void codecipbus_w_adc_en(uint32_t reg_base, uint32_t v)
{
    uint32_t val = 0;
    val = codecip_read32(reg_base, CODECIP_CR_REG_OFFSET);
    if (v)
...
static inline void codecipbus_w_sdac_rvolume_sel(uint32_t reg_base, uint32_t v)
{
    uint32_t val = 0;
    val = codecip_read32(reg_base, CODECIP_CTRL1_REG_OFFSET);
    val &= ~CODECIP_CTRL2_SDAC_R_VOL_MASK;
    if (v)
        val |= (CODECIP_CTRL2_SDAC_R_VOL_MASK & (v << CODECIP_CTRL2_SDAC_R_VOL_SHIFT));
    codecip_write32(val, reg_base, CODECIP_CTRL1_REG_OFFSET);
}
