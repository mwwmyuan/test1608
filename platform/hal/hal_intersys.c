...
static struct HAL_INTERSYS_MSG_T *** const bt_recv_msg_list_ppp[HAL_INTERSYS_ID_QTY] = {
    (struct HAL_INTERSYS_MSG_T ***)0xC0000004,
    (struct HAL_INTERSYS_MSG_T ***)0xC000000C,
};

static struct HAL_INTERSYS_MSG_T ** const recv_msg_list_p[HAL_INTERSYS_ID_QTY] = {
    (struct HAL_INTERSYS_MSG_T **)0xC0000000,
    (struct HAL_INTERSYS_MSG_T **)0xC0000008,
};
...
static struct BTCMU_T * const btcmu = (struct BTCMU_T *)0xD0330000;
...
static void hal_intersys_busy(enum HAL_INTERSYS_ID_T id, bool busy)
{
    ...
    if (busy) {
        hal_sysfreq_req(HAL_SYSFREQ_USER_INTERSYS, HAL_CMU_FREQ_26M);
        busy_now = true;
    } else {
        new_state = false;
        ...
        if (!new_state) {
            hal_sysfreq_req(HAL_SYSFREQ_USER_INTERSYS, HAL_CMU_FREQ_32K);
            busy_now = false;
        }
    }
}
...
static int hal_intersys_peer_irq_set(enum HAL_INTERSYS_ID_T id, enum HAL_INTERSYS_IRQ_TYPE_T type)
{
    ...
    hal_cmu_get_address()->ISIRQ_SET = value;
    return 0;
}
                      ...
                      HAL_INTERSYS_RX_IRQ_HANDLER rxhandler, HAL_INTERSYS_TX_IRQ_HANDLER txhandler, bool rx_flowctrl)
{
    ...
    if (!chan_opened[id]) {
        if (id == HAL_INTERSYS_ID_0) {
            hal_cmu_get_address()->ISIRQ_CLR = CMU_BT2MCU_DATA_DONE_CLR | CMU_MCU2BT_DATA_IND_CLR;
        } else {
            hal_cmu_get_address()->ISIRQ_CLR = CMU_BT2MCU_DATA1_DONE_CLR | CMU_MCU2BT_DATA1_IND_CLR;
        }
