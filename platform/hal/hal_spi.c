...
static struct SPI_T * const spi[HAL_SPI_ID_QTY] = {
    (struct SPI_T *)0x4001E000,
    (struct SPI_T *)0x40006000,
    (struct SPI_T *)0x40007000,
};
...
static bool BOOT_BSS_LOC in_use[HAL_SPI_ID_QTY] = { false, false, false, };
...
static const uint32_t ssp_freq = 26 * 1000 * 1000;
...
int hal_spi_init_ctrl(const struct HAL_SPI_CFG_T *cfg, struct HAL_SPI_CTRL_T *ctrl)
{
    ...
    ASSERT(cfg->rate <= ssp_freq / (MIN_CPSDVSR * (1 + MIN_SCR)), "SPI rate too large: %u", cfg->rate);
    ASSERT(cfg->rate >= ssp_freq / (MAX_CPSDVSR * (1 + MAX_SCR)), "SPI rate too small: %u", cfg->rate);
...
static int hal_spi_dma_send_id(enum HAL_SPI_ID_T id, const void *data, uint32_t len, HAL_SPI_DMA_HANDLER_T handler)
{
    ...
    spi_txdma_chan[id] = hal_gpdma_get_chan(id == HAL_SPI_ID_INTERNAL?HAL_GPDMA_ISPI_TX:HAL_GPDMA_ISPI_TX, HAL_DMA_HIGH_PRIO);
    if (spi_txdma_chan[id] == HAL_DMA_CHAN_NONE) {
        int_unlock(lock);
        return -4;
    }
    ...
    if (id == HAL_SPI_ID_INTERNAL) {
        dma_cfg.dst_periph = HAL_GPDMA_ISPI_TX;
    } else {
        dma_cfg.dst_periph = HAL_GPDMA_SPI_TX;
    }
...
static int hal_spi_dma_recv_id(enum HAL_SPI_ID_T id, const void *cmd, void *data, uint32_t len, HAL_SPI_DMA_HANDLER_T handler)
{
    ...
    spi_rxdma_chan[id] = hal_gpdma_get_chan(id == HAL_SPI_ID_INTERNAL?HAL_GPDMA_ISPI_RX:HAL_GPDMA_ISPI_RX, HAL_DMA_HIGH_PRIO);
    ...
    if (id == HAL_SPI_ID_INTERNAL) {
        dma_cfg.src_periph = HAL_GPDMA_ISPI_RX;
    } else {
        dma_cfg.src_periph = HAL_GPDMA_SPI_RX;
    }
