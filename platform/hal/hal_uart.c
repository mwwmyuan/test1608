...
#define UART_FREQ                       26000000
#define UART_FREQ_CRYSTAL_40M           40000000

static struct UART_T * const uart[HAL_UART_ID_QTY] = {
    (struct UART_T *)0x40008000,
    (struct UART_T *)0x40009000,
#ifdef BT_UART
    (struct UART_T *)0xd0300000,
#endif
};
...
int hal_uart_open(enum HAL_UART_ID_T id, const struct HAL_UART_CFG_T *cfg)
{
    ...
    if (hal_cmu_clock_get_status(uart_mod[id].apb) == HAL_CMU_CLK_ENABLED &&
...
void hal_uart_flush(enum HAL_UART_ID_T id)
{
    ASSERT(id < HAL_UART_ID_QTY, err_invalid_id, id);
    // Flush FIFO
    uart[id]->UARTLCR_H &= ~UARTLCR_H_FEN;
                      ...
                      struct HAL_DMA_DESC_T *desc, unsigned int *desc_cnt)
{
    ...
    recv_dma_chan[id] = hal_gpdma_get_chan((id == HAL_UART_ID_0) ? HAL_GPDMA_UART0_RX : HAL_GPDMA_UART1_RX, HAL_DMA_HIGH_PRIO);
    if (recv_dma_chan[id] == HAL_DMA_CHAN_NONE) {
        int_unlock(lock);
        return 2;
    }
    ...
    dma_cfg.src_periph = (id == HAL_UART_ID_0) ? HAL_GPDMA_UART0_RX : HAL_GPDMA_UART1_RX;
                      ...
                      struct HAL_DMA_DESC_T *desc, unsigned int *desc_cnt)
{
    ...
    send_dma_chan[id] = hal_gpdma_get_chan((id == HAL_UART_ID_0) ? HAL_GPDMA_UART0_TX : HAL_GPDMA_UART1_TX, HAL_DMA_HIGH_PRIO);
    if (send_dma_chan[id] == HAL_DMA_CHAN_NONE) {
        int_unlock(lock);
        return 2;
    }
    ...
    dma_cfg.dst_periph = (id == HAL_UART_ID_0) ? HAL_GPDMA_UART0_TX : HAL_GPDMA_UART1_TX;
    dma_cfg.dst_width = HAL_DMA_WIDTH_BYTE;
