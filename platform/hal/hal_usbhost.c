...
uint32_t usbhost_trmask = (1 << 0); //~0; //(1 << 3) | (1 << 4);
...
#define RXFIFOSIZE (2 * (MAX_USBHOST_PACKET_SIZE / 4 + 2))
    ...
    if (prt & USBC_HPRT_PRTENCHNG) {
        if (prt & USBC_HPRT_PRTENA) {
            ...
            if (speed == 1) {
                ...
                // Notify upper layer
                if (port_handler) {
                    port_handler(HAL_USBHOST_PORT_EN_FS);
                }
            } else {
...
void hal_usbhost_detect(enum HAL_USBHOST_PLUG_STATUS_T status, HAL_USBHOST_PLUG_HANDLER handler)
{
    ...
    if (handler && pmu_status != PMU_USB_PIN_CHK_NONE) {
        pmu_usb_enable_pin_status_check(pmu_status, hal_usbhost_pin_status_change);
    } else {
...
void hal_usbhost_sleep(void)
{
    ...
    if (usbhost_opened) {
        ...
        hal_cmu_clock_disable(HAL_CMU_MOD_H_USBC);
        hal_cmu_clock_disable(HAL_CMU_MOD_O_USB);
        pmu_usb_enable_pin_status_check(PMU_USB_PIN_CHK_DEV_DISCONN, hal_usbhost_pin_status_change);
    }
}
